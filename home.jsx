...
export default function App() {
  ...
  async function generateRoadmap(ans) {
    setScreen("generating"); setGenError(null); setGenStatus("Analysing your learning profile...");
    ...
    const userMsg=`Topic: ${ans.topic}\nGoal: ${ans.goal}\nWeekly time: ${ans.hoursPerWeek}\nAvailable resources: ${resList}\n\nCreate a complete learning roadmap for me.`;
    try {
      const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:4000,system:SYSTEM_PROMPT,messages:[{role:"user",content:userMsg}]})});
      timers.forEach(clearTimeout);
      if(!res.ok) throw new Error(`API error ${res.status}`);
      const data=await res.json();
      const raw=data.content?.find(b=>b.type==="text")?.text||"";
      const parsed=JSON.parse(raw.replace(/```json|```/g,"").trim());
      setRoadmap(parsed);setChecked({});setActivePhase(null);setExpandedTrack(null);setEditMode(false);setScreen("dashboard");
    } catch(e) { timers.forEach(clearTimeout); setGenError(e.message||"Something went wrong."); setScreen("intake"); }
  }
